...
export default function BiomarkerPhaseHeatmap({ data = [], theme = 'dark' }) {
  const isDark = theme === 'dark';
  const option = useMemo(() => {
    if (!data.length) return {};
    const phases = PHASE_ORDER.filter(phase => data.some(d => d.phase === phase));
    const biomarkers = [...new Set(data.map(d => d.biomarker))];
    const maxVal = Math.max(...data.map(d => d.count), 1);
    const heatData = data.map(d => [phases.indexOf(d.phase), biomarkers.indexOf(d.biomarker), d.count]).filter(d => d[0] >= 0 && d[1] >= 0 && d[2] > 0);
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: p => {
          return `<div style="font-weight:700;font-size:12px;margin-bottom:4px">${biomarkers[p.value[1]]} × ${phases[p.value[0]]}</div>` +
      ...
      grid: { left: 132, right: 28, top: 14, bottom: 85 },
      xAxis: {
        type: 'category',
        data: phases,
